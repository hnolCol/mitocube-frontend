 ...
 */
async function getFilters_API({proteome_id}){
    //fetch availabe features from the API. Reconsider /details 
    const res = await axios.get("/api/filters", {params : {proteome_id}} )
    return res.data
}

export function useGetFilters (APIParams = {proteome_id}, useQueryOptions = {}) {
    return useQuery(["getFilters",APIParams.proteome_id],() =>  getFilters_API({...APIParams}), useQueryOptions)
}
