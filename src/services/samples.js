...
export function constructSampleNames(submission_label, sampleNumber, sampleAttributes) {
    ...
    return _.range(sampleNumber).map(idx => {
        ...
        let sampleAttributeString = _.join(_.keys(sampleAttributes[idx]).map(attributeTag => _.join(sampleAttributes[idx][attributeTag].map(attrValue => attrValue.value),"-")).filter(v => v!==""),"_")
        return `${date}_${submission_label}_${sampleNumber}_${sampleAttributeString}`.replaceAll(" ","")
    }
    )
}