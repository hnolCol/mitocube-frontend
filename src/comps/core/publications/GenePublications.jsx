import { useGetPubmedArticlesByQuery_EX } from "../../../hooks/external_queries/pubmed.articles"
...
export function GenePublications({ gene_name }) {
    ...
    const { data: publications, isSuccess, isLoading, isFetching } = useGetPubmedArticlesByQuery_EX({ query : gene_name, limit : 15 }, {enabled : _.isString(gene_name)})
    return (<div>
        <h3>Publications</h3>
        {isSuccess && _.isObject(publications.esearchresult) ? <div>
        ...
        </div> : isLoading || isFetching ? <Loading /> : null  
        }
    </div>)
}